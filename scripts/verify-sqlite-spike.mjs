...
const requiredHeartbeatSampleCount = 5;
...
if (smoke.heartbeatSampleCount !== requiredHeartbeatSampleCount) throw new Error(`Packaged SQLite smoke must contain exactly ${requiredHeartbeatSampleCount} heartbeat samples per driver`);
for (const candidate of smoke.candidates) {
  if (candidate.status !== "COMPLETE" || candidate.samples?.length !== requiredHeartbeatSampleCount) throw new Error(`Packaged ${candidate.driver} smoke is incomplete`);
